...
export default function MetricsPage() {
  ...
  const fetchMetrics = async () => {
    try {
      ...
      // Use your existing endpoint that returns all ofertas
      ...
      // Transform raw ofertas into metrics data structure
      const metrics = processOfertasToMetrics(ofertas);
      setMetrics(metrics);
    } catch (error) {
  ...
  // Process raw ofertas data into metrics format
  const processOfertasToMetrics = (ofertas) => {
    // Initialize counters
    const abiertas = ofertas.filter(o => o.estadoOferta === 'ABIERTA').length;
    const cerradas = ofertas.filter(o => o.estadoOferta === 'CERRADA').length;
    const enRevision = ofertas.filter(o => o.estadoOferta === 'EN REVISION').length;
    const canceladas = ofertas.filter(o => o.estadoOferta === 'CANCELADA').length;
    const bloqueadas = ofertas.filter(o => o.estadoOferta === 'BLOQUEADA').length;
    
    // Count total interested students (flattened from all offers)
    const interesados = ofertas.reduce((sum, oferta) => 
      sum + (oferta.estudiantesInteresados?.length || 0), 0);
    
    // Count accepted students
    const aceptados = ofertas.reduce((sum, oferta) => 
      sum + (oferta.estudiantesInteresados?.filter(e => e.aceptado)?.length || 0), 0);
  ...
  const getChartData = () => {
    ...
    // Now estatusData will be properly defined
